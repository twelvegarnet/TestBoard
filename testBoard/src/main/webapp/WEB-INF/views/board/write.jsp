...
<div class="content">
	...
	<form action="/board/write" method="POST" enctype="multipart/form-data">
		...
		<div id="fileUploadBtn">
			<!-- <button type="button" onclick="inputImg()">파일 첨부</button> -->
...
function filechk(event){
	...
	if(fileSize >= 3145728){
		alert("파일 1개당 3MB 이내의 파일만 첨부 가능합니다.");
		$("#file"+fileCount).val("");
		return false;
	}
...
function setThumbnail(event) {
	for (var image of event.target.files) {
		...
		reader.onload = function(event) {
			...
			deleteBtn.setAttribute("style", "position: relative; top: -15px; left: -53px; cursor: pointer;");
...
function deleteImg(fileCount){
	$("#img"+fileCount).remove();
	$("#deleteBtn"+fileCount).remove();
